...
基于 **FastAPI** 的实时财经新闻展示系统。从 6 个财经信息源（新浪财经、财联社、同花顺、东方财富、GDELT、雅虎财经）异步并发获取最新新闻，前端 3 秒轮询 + DOM 差异渲染，实现准实时展示。
...
- **无缓存阻塞**：每次 `/api/news` 请求都真实调用 6 个 API 源
- **增量更新**：通过 `new_hashes` 字段做 DOM 差异渲染，新新闻自动显示 `[新增]` 标签
- **并发抓取**：6 个源同时请求，总耗时 ≈ 最慢那个（8s timeout）
- **前端轮询**：3 秒一次，页面不可见时暂停
